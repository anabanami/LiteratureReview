...
\captionof{figure}{Illustration of the conditions in equations \ref{eq:2.2}. \PT-symmetry and the reality of eigenvalues. This figure is quite subtle. Unbroken or exact \PT-symmetry requires that both conditions in \ref{eq:2.2} are true. Since the \PT\:operator is antilinear it is possible that eigenstates of a Hamiltonian are not also eigenstates of the \PT operator. This figure is strongly inspired by figure 2 in reference \cite{Faria1}}
...
The effectiveness of \PT\:symmetry as a tool to investigate the spectra of some non-Hermitian Hamiltonians has been shown in various works, such as Dorey et al\cite{Dorey_2001, Dorey_2004}, Bender and Boettcher\cite{RealSpectrainNHH}, Brody\cite{Brody_2016}, Bender and Mannheim \cite{Bender_2010}, Bender et al\cite{PTsymmetricQM} among others.
...
\end{Figure}
In chapter \ref{MINE} we will investigate the unbroken symmetry region of figure \ref{fig:parametricfam}, specifically the case for $\epsilon = 2$. However since in this section we are concerned with EPs, we must zoom into the parametric region $\epsilon < 0$. The broken \PT-symmetry region is visible in more detail in figure \ref{fig:broken_region}.
\begin{Figure}
\centering
\includegraphics[width=0.6\linewidth]{broken_region.pdf}
\captionof{figure}{This figure is my version of Figure 3. in reference \cite{BrokenRegion}, Similarly to figure \ref{fig:parametricfam}, this is the eigenvalue spectra of the Hamiltonians in \ref{eq:3.1} as a function of the real parameter $\epsilon$ in the case where $\epsilon < 0$. This figure displays a discretised  parameter space for $\epsilon$ as 100 values in the horizontal axis, and the energy range $-2 \leq E \leq 12$ as the vertical axis. This figure depicts fully real eigenvalues as black dots. Visible here are three epsilon values at which the real eigenvalues become degenerate (coalesce) and then form complex-conjugate pairs. The real parts of these pairs of eigenvalues are represented as blue dots. These energies initially decrease as $\epsilon$ decreases but blow up suddenly as $\epsilon$ approaches $-1$ (The blow up of the real-parts was not captured by my numerics). The imaginary parts of the eigenvalue pairs are represented as red dots. These remain finite and appear to decay to zero at $\epsilon = -1$.}
...
When physicists test behaviour of states and eigenvalues under EPs is common practice to encircle the EP on a closed contour in parameter space. For examples see chapter \ref{Experiments}. There are cases of EPs where eigenvalues and the wave functions are interchanged, For example, if the EP corresponds to a cubic root branch point. For a closed contour around this point a cyclic permutation of all three eigenvalues and eigenfunctions is then expected\cite{Cartarius}.
...
Currently I am working in simulating the dynamics of a single trapped state after we implement a quench that would instantaneously change the trapping potential from a harmonic oscillator trap to the potential present in the Hamiltonian of equation \ref{eq:6.1}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Quantum simulation:\\ Time evolution}
...
\captionof{figure}{Eight frames in a time evolution simulation written using RK4. The figure should be read from top left to bottom right. This simulation implements time evolution by repeatedly solving the Schr\"{o}dinger equation for the Hamiltonian in \ref{eq:6.1} using the iterative RK4 algorithm. From the earliest frame (top left) to the latest frame (bottom right) a refocusing of the state can be seen as the state is allowed to evolve in the negative quartic potential trap.}
\label{fig:RK4}
\end{Figure}
...
\captionof{figure}{Three frames from a time evolution simulation written using a unitary operator made using the Hermitian Hamiltonian  in \ref{eq:6.2}. The state trapped in this potential simply shifts back an forth along the x-axis but no other phenomena are observed as time passes.}
\label{fig:EHH_sim}
\end{Figure}
The simulation in figure \ref{fig:EHH_sim} gave us some insight in how equivalent energy spectra of certain non-Hermitian Hamiltonians and certain Hermitian Hamiltonians does not guarantee that the time evolution of states under these Hamiltonians will be equivalent or even similar.
\section{Quantum simulation:\\ \PT-symmetric time evolution}
...
The \emph{Stokes wedges} for the Hamiltonians in \ref{eq:3.1} are always centred about the origin of the complex x-plane. Their angular orientation of these wedges in the complex x-plane can be described by the argument of the complex variable $x = re^{i\theta}$.
