...
\end{align}
If we ``swap" Hermiticity: $\tilde{H}^{\dagger}$ in \ref{eq:13} for \PT-symmetry, and we use equation \ref{eq:11}
...
\section{Interlude: A toy example}\label{Inter}
I am interested in the applications of \PT-symmetry. My project focuses in dynamics under non-Hermitian but \PT-symmetric Hamiltonians. In this section I illustrate some interesting differences between Hermitian and non-Hermitian systems.
\subsection{Two $2\times2$ Hamiltonians}
In \cite{Bender_2007} ``Faster than Hermitian Quantum mechanics" Bender et al compare a Hermitian Hamiltonian $\hat{H}$ and a \PT-symmetric non-Hermitian Hamiltonian $\tilde{H}$
\begin{align}\label{eq:13}
\hat{H} = \begin{pmatrix}
s & re^{-i\theta}\\
re^{i\theta} & u \\
