...
% In chapter \ref{CPT} we stablished a positive definite norm in the \PT-symmetric framework.

%  The constancy of the norm is an essential feature of a quantum system because the norm of a state is a probability, and this probability must remain constant in time. If this probability were to grow or decay in time, we would say that the theory violates unitarity. In \PT-symmetric quantum mechanics $\hat{H}$ is not Dirac Hermitian, but the norms of states are still time independent\cite{MakingSense}, and as explained in the previous section, when the \PT-symmetry of $\hat{H}$ is unbroken we still have positive norm states.
