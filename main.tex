...
\mathcal{T}:& \quad\hat{x} \rightarrow \hat{x},\quad\quad \hat{p} \rightarrow -\hat{p},\quad i \rightarrow -i.
\end{split}
\end{equation}
...
\captionof{figure}{Pictorial representation of the two types of Hilbert space geometries and the time evolution from an up state to a down state in a two level system described by two distinct Hamiltonians. In Ref.~\cite{Bender_2007} the shortest possible time in which state $\ket{\psi_i}$ can evolve to state $\ket{\psi_f}$ is shown to be a quantity dependent on $\omega$, which is the energy difference between eigenvalues of the two Hamiltonians. Bender et al. show in their paper that if an identical constraint is imposed on the $\omega$ values for each distinct Hamiltonian, they will describe very different geometries for each Hilbert space. Ref.~\cite{Bender_2007} shows how a spin-up state $\ket{\psi_i}$ can be transformed arbitrarily quickly to a spin-down state by a general non-Hermitian Hamiltonian because the geometry of its Hilbert space is is hyperbolic. In contrast, the Hilbert space geometry for a general Hermitian Hamiltonian is elliptic, making the shortest possible time for evolution in the Hermitian framework non-zero.}
\label{fig:HGeo}
\end{Figure}
...
Fig.~\ref{fig:HGeo}. The time evolution under the two Hamiltonians theorized in Ref.~\cite{Bender_2007} occurs very differently, because only in the non-Hermitian Hamiltonian case the Hilbert space can be made so that the initial and final states lie on top of each other, essentially making the time evolution instantaneous.
