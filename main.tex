...
Any non-Hermitian Hamiltonian with unbroken \PT-symmetry can be converted to a spectrally equivalent Hermitian Hamiltonian by means of a ``similarity transformation"\cite{PTvsDH}\cite{Mostafazadeh2}.
The nature and possible advantages of this fact are explored in references \cite{Mostafazadeh},\cite{EquivalentHH},\cite{Pseudo-HermiticityIII},\cite{Jones_2005},\cite{taleof2potentials}.
In \cite{Mostafazadeh}, Mostafazadeh conjectures that because \CPT-symmetry satisfies the postulates of quantum mechanics --whilst only ``swapping" the Hermiticity of a Hamiltonian by \CPT-symmetry-- then non Hermitian \CPT-symmetric theories are equivalent to certain non local Hermitian field theories. It is natural to notice that this feature could provide an advantage to simplify quantum mechanical calculations.
